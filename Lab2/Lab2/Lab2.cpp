...
void OnPaint(HWND hwnd)
{
	...
	if (g_pBitmap.get() != NULL)
	{
		...
		float imageWidth = g_pBitmap->GetWidth();
		float imageHeight = g_pBitmap->GetHeight();
		RECT rect;
		...
		float left = (windowWidth - imageWidth) / 2;
		float top = (windowHeight - imageHeight) / 2;
		 
		g.DrawImage(g_pBitmap.get(), REAL{left}, REAL{top}, REAL{imageWidth}, REAL{imageHeight});
	}
