...
void CRaytraceView::AddSomeSpheres()
{
	...
	sphereTransform1.Translate(-4, -2, -20);
	CMatrix4d sphereTransform2;
	sphereTransform2.Translate(4, 2, 0);
	...
	AddSphere(shader, 1, CVector3d(-4, -2, -20));
	AddSphere(shader, 1, CVector3d(4, 2, 0));
}

void CRaytraceView::AddPyramidToy()
{
	CSimpleMaterial yellow;
	yellow.SetDiffuseColor(CVector4f(1, 1, 0, 1));
	...
	orange.SetDiffuseColor(CVector4f(1, 0.5, 0, 1));
	CSimpleDiffuseShader& shaderYellow = CreateSimpleDiffuseShader(yellow);
	CSimpleDiffuseShader& shaderWhite = CreateSimpleDiffuseShader(white);
	CSimpleDiffuseShader& shaderGreen = CreateSimpleDiffuseShader(green);
	CSimpleDiffuseShader& shaderRed = CreateSimpleDiffuseShader(red);
	CSimpleDiffuseShader& shaderBlue = CreateSimpleDiffuseShader(blue);
	CSimpleDiffuseShader& shaderOrange = CreateSimpleDiffuseShader(orange);
	CMatrix4d rotateTransform;
	rotateTransform.Rotate(-20, 1, 0, 0);
	AddTorus(shaderWhite, 1, 0.25, CVector3d(0, -3, 0), rotateTransform);
	AddTorus(shaderYellow, 0.9, 0.25, CVector3d(0, -2.5, 0), rotateTransform);
	AddTorus(shaderGreen, 0.8, 0.25, CVector3d(0, -2, 0), rotateTransform);
	AddTorus(shaderRed, 0.7, 0.25, CVector3d(0, -1.5, 0), rotateTransform);
	AddTorus(shaderBlue, 0.6, 0.25, CVector3d(0, -1, 0), rotateTransform);
	AddTorus(shaderOrange, 0.5, 0.25, CVector3d(0, -0.5, 0), rotateTransform);
	AddSphere(shaderRed, 0.5, CVector3d(0, 0, 0), rotateTransform);
}
...
CSceneObject& CRaytraceView::AddTorus(IShader const& shader, double radius, double tubeRadius, CVector3d const& center, CMatrix4d const& transform)
{
	const auto& sphere = *m_geometryObjects.emplace_back(
		std::make_unique<CTorus>(radius, tubeRadius, center, transform));
